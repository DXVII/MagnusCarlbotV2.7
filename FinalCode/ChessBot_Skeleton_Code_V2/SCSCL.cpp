...
void SCSCL::SyncWriteVel(u8 ID[], u8 IDN, u16 Position, u16 Time[], u16 Speed)
{
    //u8* offbuf[IDN];
    u8 offbuf[IDN][6];
    for(u8 i = 0; i<IDN; i++){
...
void SCSCL::ReadPosMulti(u8 *ID, u8 IDN, s16 *pos)
{
    
    for (int i=0; i < IDN; i++) {
        pos[i] = readWord(ID[i], SCSCL_PRESENT_POSITION_L);
    }
    
}


// Read Velocity - NGB 11/12/20
s16 SCSCL::ReadVel(u8 ID, u8 *Err)
{
    s16 Vel = readWord(ID, SCSCL_PRESENT_SPEED_L);
    if(Vel==-1){
        if(Err){
            *Err = 1;
        }
        ...
        return Vel + 32768; // Speed reading offset when rotating CCW
    }else{
        if(Err){
            *Err = 0;
        }
...
// Read Current - NGB 30/11/20
int SCSCL::ReadCur(u8 ID, u8 *Err)
{
    int cur = readWord(ID, SCSCL_PRESENT_CURRENT_L);
    if(cur==-1){
        if(Err){
            *Err = 1;
        }
